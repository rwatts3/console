...
export default class RelationInfo extends React.Component<Props, State> {
  ...
  render() {
    return !this.state.expanded
      ? <div className="flex justifyEnd pr16 pb16 mt10 h100">
          <div
            className={`pointer ${(!this.props.leftModel ||
  ...
  private generateFirstInfoSentence = (): JSX.Element => {
    ...
    const firstModel = cardinality.startsWith('ONE')
      ? leftModel.name
      : leftModel.namePlural
    const secondModel = cardinality.endsWith('ONE')
      ? rightModel.name
      : rightModel.namePlural
    return (
  ...
  private generateSecondInfoSentence = (): JSX.Element => {
    ...
    const secondModelName = cardinality.endsWith('MANY')
      ? rightModel.namePlural
      : rightModel.name
    return (
      <div className="infoLine">
  ...
  private generateThirdInfoSentence = (): JSX.Element => {
    ...
    const secondModelName = cardinality.startsWith('MANY')
      ? leftModel.namePlural
      : leftModel.name
    return (
      <div className="infoLine">
