...
class EmptyRow extends React.Component<Props, State> {
  ...
  render() {
    const borderColor = this.state.isEnteringEmail
      ? 'blueBorder'
      : this.props.hasAddFunctionality ? $p.bBlack20 : $p.bBlack10
      ...
      rowContent = (
        ...
        >
          <style jsx={true}>{`
            ...
            .grayCircle {
              @inherit: .flex, .justifyCenter, .itemsCenter, .br100, .mr16,
                .hS25, .wS25, .bgBlack07;
            }
          `}</style>
          ...
          <div className="f16 black40">
            add collaborator (
            {numberOfLeftSeats < 0 ? 'unlimited' : numberOfLeftSeats}{' '}
            {numberOfLeftSeats === 1 ? 'seat ' : 'seats '}
            left)
          </div>
        </div>
      )
    ...
    InviteCollaboratorMutation.commit({
      ...
      .then(() => {
        ...
        this.props.showNotification({
          message: 'Added new collaborator: ' + email,
          level: 'success',
        })
      })
      .catch(transaction => {
