...
export default class MemberRow extends React.Component<Props, State> {
    ...
    return (
      ...
      >
        <style jsx>{`
          ...
          .seatIndicator {
            @p: .flex, .justifyCenter, .itemsCenter, .br100, .wS38, .hS38, .fw6,
              .f20;
          }
        ...
        <div className={`flex justifyBetween itemsCenter w100 ph10`}>
          <div className="flex itemsCenter">
            <div className={`seatIndicator ${this.randomColor()}`}>
              {this.props.seat.name
                ? this.props.seat.name.charAt(0)
                : this.props.seat.email.charAt(0)}
            </div>
            <div className="pl16 black80 f20 fw3">
              {this.props.seat.name}
            </div>
            <div className="pl16 black40 f16 fw6">
              {this.props.seat.email}
            </div>
            {this.isCurrentUser() &&
              <div className="pl2 black50 f20 fw3">(you)</div>}
            {this.props.seat.status === 'INVITED_TO_PROJECT' &&
              <div className="pl2 black50 f20 fw3">(invited)</div>}
          </div>
          {this.getRightPart()}
        </div>
      </div>
  ...
  private getRightPart = (): JSX.Element => {
    if (this.props.seat.isOwner) {
      return (
        <div className="f14 fw6 ttu itemsStart green bgLightgreen20 br2 pv2 ph6">
          Owner
        </div>
      )
