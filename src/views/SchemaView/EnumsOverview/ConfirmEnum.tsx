...
export default class ConfirmModel extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <div
        className={`container ${this.props.delete
          ? 'deletePositioning'
      ...
      >
        ...
        `}</style>
        <div
          className={`flex itemsCenter pv10 pl10 ${this.props.delete
            ? 'headerColorsRed'
            : 'headerColorsOrange'}`}
        >
          <Icon
        ...
        <div className="pa25 f16 black50 bb bBlack10">
          {this.props.delete
            ? <div>
                You are deleting the model <b>{initialModelName}</b>. This will
                result in data loss.
              </div>
            : <div>
                You are renaming the model <b>{initialModelName}</b> to{' '}
                <b>{mutatedModelName}</b>. Please be aware, that this could
      ...
      <div className="flex justifyBetween bgBlack04">
        ...
        {this.props.initialModelName === this.state.enteredModelName
          ...
          : <input
              ...
              onChange={(e: any) =>
                this.setState({ enteredModelName: e.target.value } as State)}
              autoFocus={true}
            />}
      </div>
    )
