      ...
      <div className={'field-item' + (hideBorder ? '' : ' show-border')}>
        ...
        <div className="flexy">
          ...
          {field.isSystem &&
            <Info
              customTip={
                <Icon src={require('assets/icons/lock.svg')} className="lock" />
              }
              offsetX={30}
              cursorOffset={-5}
            >
        ...
        <div className="flexy">
          <div>
            {field.isUnique &&
              <Info
                customTip={<div className="unique">Unique</div>}
                width={180}
                offsetX={-40}
              >
                <div className="wrap">
          ...
          {!create &&
            <Link to={`/${projectName}/permissions`}>
              <PermissionsTag permissions={permissions} />
            </Link>}
        </div>
      </div>
    )
    ...
    permissions
      ...
      .forEach(permission => {
        const appliesToFields =
          permission.applyToWholeModel || permission.fieldIds.includes(field.id)
        permissionMap[permission.operation] =
          permissionMap[permission.operation] || appliesToFields
      })
