...
export default class QueryHistory extends React.Component<Props, State> {
  ...
  render() {
    if (this.state.queries.length === 0) {
      return <div className={classes.root}>No history yet</div>
    }
        ...
        <div className={classes.root}>
          <div className={classes.list}>
            {this.state.queries.map((query, index) =>
              <div
                key={index}
                className={`${classes.query} ${this.state.selectedIndex ===
                index
                  ? classes.querySelected
                  : ''}`}
                onMouseEnter={() =>
          ...
          >
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: this.state.queries[this.state.selectedIndex].query,
              }}
            />
            <br />
            Variables:
            <div
              className={classes.detailsQuery}
              dangerouslySetInnerHTML={{
                __html: this.state.queries[this.state.selectedIndex].variables,
              }}
            />
          </div>
        </div>
