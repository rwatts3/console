...
class RelationExplanation extends React.Component<Props, {}> {
  render() {
    const {
      fieldOnLeftModelIsList,
      leftModelId,
      fieldOnRightModelIsList,
      rightModelId,
    } = this.props
    ...
    const leftModelName = fieldOnRightModelIsList
      ? getModelNamePlural(leftModelId, models)
      : getModelName(leftModelId, models)
    const leftNameS =
      getModelName(leftModelId, models).slice(-1) === 's' ? `'` : `'s`

    const rightModelMultiplicity = fieldOnRightModelIsList ? 'Many' : 'One'
    const rightModelName = fieldOnLeftModelIsList
      ? getModelNamePlural(rightModelId, models)
      : getModelName(rightModelId, models)
    const rightNameS =
      getModelName(rightModelId, models).slice(-1) === 's' ? `'` : `'s`
    return (
      <div className={classes.root}>
        <div className={classes.sentence}>
          <span className={classes.multiplicity}>
            {rightModelMultiplicity}
          </span>{' '}
          <span className={classes.model}>{leftModelName}</span>
          {` ${rightModelMultiplicity === 'One' ? 'is' : 'are'} related to `}
          <span className={classes.multiplicity}>
            {leftModelMultiplicity.toLowerCase()}
          </span>{' '}
        ...
        {this.props.fieldOnRightModelName &&
          <div className={classes.sentence}>
            ...
            {` represents `}
            <span className={classes.multiplicity}>
              {rightModelMultiplicity.toLowerCase()}
            </span>{' '}
            <span className={classes.model}>{leftModelName}</span>
          </div>}
      </div>
