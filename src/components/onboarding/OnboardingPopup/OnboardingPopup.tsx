...
class OnboardingPopup extends React.Component<Props, {}> {
  render() {
    return (
      ...
      >
        <div
          className="bg-white br-2 flex shadow-2"
          style={{ maxWidth: 'calc(90vw - 266px)' }}
        >
          <div className="w-70 pa-60 tc fw1">
            <div className="ttu" style={{ letterSpacing: 2 }}>
              Getting Started
            </div>
            ...
            <div className="lh-1-4 mv-16">
              Hi {this.props.firstName}, let's get started by building a backend
              for a simple Instagram clone. To keep our example light, our
              Instagram posts only consist of a picture and some hashtags. After
              setting up the schema and creating some example posts, we are
              ready to put the backend to work and query all posts that contain
              a specific hashtag.
            </div>
  ...
  private skipGettingStarted = (): void => {
    graphcoolConfirm('You are skipping the getting started tour.').then(() => {
      Promise.resolve(this.props.skip()).then(() => {
