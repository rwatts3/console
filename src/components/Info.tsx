...
const Info = (props: Props) => {
  ...
  const cursorOffset =
    -(props.offsetX || 0) + (props.cursorOffset ? props.cursorOffset : 0)

  const manual = typeof props.isOpen === 'boolean'

  return (
    <div className={cn('info', { manual, open: props.isOpen })}>
      <style jsx>{`
        .question-mark {
          @p: .bgBlack10, .flex, .itemsCenter, .justifyCenter, .black40, .f12,
            .fw6, .br100, .pointer, .ml10;
          width: 18px;
          height: 18px;
        }
      ...
      {props.customTip
        ...
        : <div
            className={
              'question-mark' + (Boolean(props.bright) ? ' bright' : '')
            }
          >
            <span>?</span>
          </div>}
