...
export default class AddCustomMutationFunction extends Relay.Mutation<Props, {}> {

  getMutation () {
    return Relay.QL`mutation{addCustomMutationFunction}`
  }

  getFatQuery () {
    return Relay.QL`
      fragment on AddCustomMutationFunctionPayload {
        function
        project
      }
    `
